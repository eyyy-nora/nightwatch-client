import type { ArrayOr, Notifiable, Subscribable, SubscribableOptions } from "src/renderer/store/simple-store/types";
...
export function createSubscribable<T extends unknown[]>({
  ...
  function subscribe(listener: ArrayOr<(...params: T) => void>) {
    ...
    if (listeners.length + wrapped.length > maxListeners) throw new Error(tooManyListeners(maxListeners));
